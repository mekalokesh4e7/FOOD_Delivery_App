...
public class Food_ItemsDAO {
	public static ArrayList<Food_ItemsDTO> getFoodList(){
		...
		try {
			...
			while(res.next()) {
				Food_ItemsDTO FoodList = new Food_ItemsDTO(res.getInt(1), res.getString(2), res.getInt(3), res.getString(4), res.getString(5), res.getFloat(6), res.getInt(7), res.getString(8));
				Food.add(FoodList);
			}
		}
		catch(Exception ex) {
